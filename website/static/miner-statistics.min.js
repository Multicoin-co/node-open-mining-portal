...
$(function(){
	...
	function updateChartData(){
		...
		for (var w in statData.history) {
			...
			var foundPPSWorker = statData.history[w][wh].hashrate === undefined;
			var foundSOLOWorker = statData.history[w][wh].solohashrate === undefined;
			for (var i = 0; i < workerHashrateData.length; i++) {
				if (!foundPPSWorker && workerHashrateData[i].key === (worker + ' (PPS)')) {
					foundPPSWorker = true;
				...
				if (foundPPSWorker && foundSOLOWorker) {
					break;
				}
			}
			if (!foundPPSWorker) {
	...
	function updateStats() {
		...
		$("#statsHashrateAvg").text(getReadableHashRateString(calculateAverageHashrate(null, 'PPS')));
