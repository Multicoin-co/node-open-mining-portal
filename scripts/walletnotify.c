...
Simple lightweight & fast - a more efficient wallet notify script in pure C.
...
Build with:
    gcc walletnotify.c -o walletnotify


Example usage in daemon coin.conf using default NOMP CLI port of 17117
    walletnotify="/bin/walletnotify 127.0.0.1:17117 dogecoin %s"
...
int main(int argc, char **argv)
{
    ...
    {
        // print help
        printf("NOMP pool wallet notify\n usage: <host:port> <coin> <block>\n");
        exit(1);
    }
	...
	snprintf(sendline, sizeof(sendline) - 1, "{\"command\":\"walletnotify\",\"params\":[\"%s\",\"%s\"]}\n", argv[2], argv[3]);
